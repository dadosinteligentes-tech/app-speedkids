...
export async function searchCustomers(db: D1Database, query: string, limit = 10): Promise<Customer[]> {
	const pattern = `%${query}%`;
	const { results } = await db
		.prepare("SELECT * FROM customers WHERE name LIKE ? OR phone LIKE ? OR email LIKE ? ORDER BY name ASC LIMIT ?")
...
export async function createCustomer(
	db: D1Database,
	params: { name: string; phone?: string; email?: string; notes?: string },
	...
	return db
		.prepare(`
			...
			RETURNING *
		`)
		.bind(params.name, params.phone ?? null, params.email ?? null, params.notes ?? null)
		.first<Customer>();
}
...
export async function updateCustomer(
	...
	return db
		.prepare(`
			UPDATE customers
			SET name = ?, phone = ?, email = ?, notes = ?, updated_at = datetime('now')
			WHERE id = ?
			RETURNING *
		`)
		.bind(
