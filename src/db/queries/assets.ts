...
export async function getAssets(db: D1Database): Promise<Asset[]> {
	const { results } = await db
		.prepare("SELECT * FROM assets WHERE status != 'retired' ORDER BY sort_order ASC, name ASC")
		.all<Asset>();
	return results;
}
...
export async function getAllAssets(db: D1Database): Promise<Asset[]> {
	const { results } = await db
		.prepare("SELECT * FROM assets ORDER BY sort_order ASC, name ASC")
		.all<Asset>();
	return results;
}
...
export async function createAsset(
	...
	return db
		.prepare(`
			INSERT INTO assets (name, asset_type, model, photo_url, notes, uses_battery, max_weight_kg, min_age, max_age, sort_order, status)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'available')
			RETURNING *
		`)
		.bind(
