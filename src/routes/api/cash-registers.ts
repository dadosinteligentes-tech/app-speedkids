...
import { openRegister, closeRegister, getOpenRegister, getRegisterById, addTransaction, getTransactions, calculateExpected, getRecentRegisters, getRegisterSummary, getDenominationInventory, saveDenominations } from "../../db/queries/cash-registers";
import { getActiveShift } from "../../db/queries/shifts";
import { auditLog } from "../../lib/logger";
import { calculateChange, denomTotal } from "../../lib/denominations";
...
cashRegisterRoutes.post("/open", async (c) => {
	...
	const body = await c.req.json<{ opening_balance_cents?: number; denominations?: Record<string, number> }>();
	const shift = await getActiveShift(c.env.DB, user.id);

	const denoms: DenominationMap | undefined = body.denominations
		? Object.fromEntries(Object.entries(body.denominations).map(([k, v]) => [Number(k), v]))
		: undefined;
...
cashRegisterRoutes.post("/:id/close", async (c) => {
	...
	const body = await c.req.json<{ closing_balance_cents?: number; denominations?: Record<string, number> }>();
	const closingDenoms: DenominationMap | undefined = body.denominations
		? Object.fromEntries(Object.entries(body.denominations).map(([k, v]) => [Number(k), v]))
		: undefined;
	const closingCents = closingDenoms ? denomTotal(closingDenoms) : (body.closing_balance_cents ?? 0);

	await closeRegister(c.env.DB, id, user.id, closingCents, closingDenoms);
...
cashRegisterRoutes.post("/:id/transactions", async (c) => {
	...
	const body = await c.req.json<{
		type: "adjustment" | "withdrawal" | "deposit";
		amount_cents?: number;
		description?: string;
	...
	const txDenoms: DenominationMap | undefined = body.denominations
		? Object.fromEntries(Object.entries(body.denominations).map(([k, v]) => [Number(k), v]))
		: undefined;
...
cashRegisterRoutes.post("/:id/calculate-change", async (c) => {
	...
	const receivedCents = denomTotal(payDenoms);
