...
shiftRoutes.get("/:id", async (c) => {
	const tenantId = c.get('tenant_id');
	const shift = await getShiftById(c.env.DB, tenantId, Number(c.req.param("id")));
...
shiftRoutes.post("/start", async (c) => {
	...
	const existing = await getActiveShift(c.env.DB, tenantId, user.id);
	if (existing) return c.json({ error: "Já existe um turno ativo" }, 400);

	const shift = await startShift(c.env.DB, tenantId, user.id, body.name);
...
shiftRoutes.post("/:id/end", async (c) => {
	...
	await endShift(c.env.DB, tenantId, id, body.notes);
	await auditLog(c, "shift.end", "shift", id);
	const shift = await getShiftById(c.env.DB, tenantId, id);
	return c.json(shift);
});
